...
opts = arg_define(varargin, ...
    ...
    arg({'timerange','TimeRange'},10,[],'Initial time range in seconds. The time range of the display window; can be changed with keyboard shortcuts (see help).'), ...
    arg({'datascale','DataScale'},2000,[],'Initial scale of the data. The scale of the data, in units between horizontal lines; can be changed with keyboard shortcuts (see help).'), ...
    arg({'channelrange','ChannelRange'},1:4,[],'Channels to display. The channel range to display.'), ...
    arg({'samplingrate','SamplingRate'},220,[],'Sampling rate for display. This is the sampling rate that is used for plotting; for faster drawing.'), ...
...
% else
    ...
    function inlet = create_inlet(opts)
        ...
        result = {};
        disp(['Looking for a stream with type=' opts.streamname ' ...']);
